...
Maintenance - Each iteration of the loop expands the subarray, but keeps the sorted property. An element V only gets inserted into the array when it is >= to the element to the left of it. Since the elements to the left have already been sorted, it means V >= all the elements to the left, so the array remains sorted. (In InsertionSort2, we saw this by printing array each time an element was ‘inserted’ into it.)
...
Details
The Input format and constraints are the same as the previous challenges, and they are repeated below.
